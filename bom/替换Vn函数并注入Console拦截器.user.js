...
(function() {
    ...
    // 检查是否已经处理过，避免无限刷新
    if (window.location.href.includes('#vn-replaced') ||
        sessionStorage.getItem('vn-replaced') ||
    ...
    // 新的Un函数定义
    const newUnFunction = `function Vn(r,n,t,e) {
    uniqueId=r;
    state=n;
    context=t;
    flag=e;
    ...
    function createNewPageFixed() {
        ...
        fetch(originalUrl)
            ...
            // 添加标记，表示页面已经处理过
            modifiedHtml = modifiedHtml.replace('<html', '<html data-vn-replaced="true"');
            ...
            // 设置标记
            sessionStorage.setItem('vn-replaced', 'true');
    ...
    function replaceUnFunctionInHtml(html) {
        const startPattern = /function\s+Vn\s*\(\s*r\s*,\s*n\s*,\s*t\s*,\s*e\s*\)\s*\{/g;
